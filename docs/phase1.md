# Phase 1: Discovery & Design — Sailpoint Jml Bridge

## Scope
This document captures the requirements-gathering, architectural design, and planning phase for sn_sailpoint_jml_bridge.
...
- `x_sn_sailpoint_jml_bridge_config`: Application configuration and properties.
- `x_sn_sailpoint_jml_bridge_scan_log`: Record of each scan execution (start, end, status, record count).
- `x_sn_sailpoint_jml_bridge_report`: Generated reports with metadata (format, size, checksum).
- `x_sn_sailpoint_jml_bridge_delta_watermark`: High-water marks for incremental scanning per table.
...
| R3 | Scope collision with other apps | Medium | Low | Unique prefix `x_sn_sailpoint_jml_bridge`; validate before install |
