# Phase 2: Build & Validate — Sailpoint Jml Bridge

## Scope
This document covers the implementation, testing, quality assurance, and packaging phase for sn_sailpoint_jml_bridge.
...
- **Repositories:** GitHub — `vladarchitectservicenow-oss/sn_sailpoint_jml_bridge`.
...
- Script Include (`sn_sailpoint_jml_bridgeProcessor`) encapsulating business rules.
...
- Watermark updates in `x_sn_sailpoint_jml_bridge_delta_watermark`.
- Report artifacts stored as attachments on `x_sn_sailpoint_jml_bridge_report`.
...
- Scripted REST API (`sn_sailpoint_jml_bridgeAPI`) with named resources.
...
- Job status tracked in `x_sn_sailpoint_jml_bridge_scan_log`.
